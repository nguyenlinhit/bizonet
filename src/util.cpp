...
const char * const BITCOIN_CONF_FILENAME = "bizonet.conf";
const char * const BITCOIN_PID_FILENAME = "bizonetd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bizonet" is a composite category enabling all bizonet-related debug output
            if(ptrCategory->count(string("bizonet"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bizonetCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bizonetCore
    // Mac: ~/Library/Application Support/bizonetCore
    // Unix: ~/.bizonetcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bizonetCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bizonetCore";
#else
    // Unix
    return pathRet / ".bizonetcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bizonet.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bizonet.conf
